  ...
  private async evaluateSingleRule(
    ...
    try {
      ...
      if (!rule.content_rules || rule.content_rules.length === 0) {
        this.logger.debug(
          `RuleEvaluator: File '${rule.pattern}' matched ${matchingFiles.length} files: ${matchingFiles.map((f) => f.filename).join(', ')}`,
        );
        return {
          matched: true,
      ...
      this.logger.debug(
        `RuleEvaluator: File '${rule.pattern}' matched ${matchingFiles.length} files, checking content rules...`,
      );
