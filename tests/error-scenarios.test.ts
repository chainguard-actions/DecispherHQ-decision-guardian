...
import { getFileDiffs, getChangedFiles } from '../src/github-utils';
import { ContentMatchers } from '../src/content-matchers';
import { DecisionParser } from '../src/parser';
import { CommentManager } from '../src/comment';
import { ContentRule } from '../src/rule-types';
import { FileDiff, DecisionMatch } from '../src/types';
...
describe('Error Handling', () => {
    describe('GitHub API Rate Limits', () => {
        it('handles 429 rate limit with retry-after header', async () => {
            ...
            const result = await getFileDiffs('fake-token', {
                owner: 'test',
                repo: 'test',
                pull_number: 1
            });
        ...
        it('fails immediately on non-rate-limit errors', async () => {
            ...
            await expect(getFileDiffs('fake-token', {
                owner: 'test',
                repo: 'test',
                pull_number: 1
            })).rejects.toThrow();
    ...
    describe('ReDoS Protection', () => {
        it('rejects unsafe regex patterns using safe-regex', async () => {
            const matchers = new ContentMatchers();
    ...
    describe('Concurrent Comment Updates', () => {
        it('handles 409 conflict with exponential backoff', async () => {
            ...
            const manager = new CommentManager('fake-token');
...
Test context
            ...
            expect(result.errors.filter(e => e.message.includes('Security')).length).toBe(0);
